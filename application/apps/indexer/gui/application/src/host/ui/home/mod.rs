...
fn draw_recent_files(
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            for file in &mut state.recent_sessions {
                ui.collapsing(file.title.clone(), |ui| {
                    let cfg_len = file.configurations.len();
                    let cfg_tpl = file.setup_template();
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if let Some(cfg) = cfg_tpl {
                                if ui
                                    ...
                                    .clicked()
                                {
                                    open_configuration(actions, cmd_tx, &cfg, true);
                                }
                            }
                        });
                    }
                    ...
                    for cfg in &file.configurations {
                        ui.horizontal(|ui| {
                            if ui
                                ...
                                .clicked()
                            {
                                open_configuration(actions, cmd_tx, &cfg, false);
                            }

                            if ui
                                .add(
                                    Button::new(RichText::new(icons::regular::MINUS).size(12.0))
                                        .small(),
                                )
...
fn open_configuration(
    ...
    match cfg.validate() {
        Ok(()) => {
            let cmd = HostCommand::OpenConfiguration((Box::new(cfg.options.clone()), setup));
