        ...
        match Message::from_slice(payload) {
            Ok(message) => {
                let consumed = match &message {
                    Message::Sd(header, _) | Message::Rpc(header, _) => {
        ...
        match reader.next() {
            Ok((offset, block)) => {
                if let PcapBlockOwned::Legacy(b) = block {
                    if let Some(transport) = extract_ethernet_payload(&b.data) {
                ...
                if let PcapBlockOwned::NG(block) = block {
                    match block {
                        Block::EnhancedPacket(epb) => {
                            if let Some(transport) = extract_ethernet_payload(epb.data.as_ref()) {
                                collect_statistics_from_transport_payload(
                                    collector, statistics, transport,
                                )?;
                            }
                        ...
                        Block::SimplePacket(spb) => {
                            if let Some(transport) = extract_ethernet_payload(spb.data.as_ref()) {
                                collect_statistics_from_transport_payload(
                                    collector, statistics, transport,
                                )?;
                            }
...
mod tests {
    ...
    fn test_statistics_from_udp_pcap() {
        let sources: Vec<PathBuf> =
            ["../../../../../developing/resources/someip/udp/example.pcap".into()].to_vec();
    ...
    fn test_statistics_from_tcp_pcap() {
        let sources: Vec<PathBuf> =
            ["../../../../../developing/resources/someip/tcp/example.pcap".into()].to_vec();
    ...
    fn test_statistics_from_udp_pcapng() {
        let sources: Vec<PathBuf> =
            ["../../../../../developing/resources/someip/udp/example.pcapng".into()].to_vec();
    ...
    fn test_statistics_from_tcp_pcapng() {
        let sources: Vec<PathBuf> =
            ["../../../../../developing/resources/someip/tcp/example.pcapng".into()].to_vec();
