...
impl HomeSettings {
    ...
    pub fn update_configurations(&mut self) {
        ...
        self.recent_sessions
            .retain(|file| !file.configurations.is_empty());
    }
...
impl RecentSession {
    /// Get a template config for new configuration setup, if supported.
    pub fn setup_template(&self) -> Option<&SessionConfig> {
        ...
        let opts = ObserveOptions {
            origin: match &options.origin {
                ObserveOrigin::File(_, format, path) => {
                    ObserveOrigin::File(String::new(), format.clone(), path.clone())
                }
                ObserveOrigin::Concat(files) => ObserveOrigin::Concat(
                    files
                        .iter()
                        .map(|(_, format, path)| (String::new(), format.clone(), path.clone()))
                        .collect(),
                ),
