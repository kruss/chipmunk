...
mod test {
    ...
    fn test_parse_prefix() {
        ...
        assert_eq!("127.0.0.1:12345 << TCP INST:1", result.1);

        let input = [0x7F, 0x00, 0x00, 0x01, 0x30, 0x39, 0x02, 0x01, 0x00, 0x02];
        let result = SomeipParser::parse_prefix(&input).expect("prefix");
        assert_eq!("127.0.0.1:12345 >> UDP INST:2", result.1);

        let input = [
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x00, 0x03,
        ];
        let result = SomeipParser::parse_prefix(&input).expect("prefix");
        assert_eq!("0.0.0.0:0 LOCAL INST:3", result.1);
